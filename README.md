...
1. **人脸库**：上传或拍摄正脸照，自动检测人脸并生成特征。一张图里有多张脸时，会按张依次添加：每张可归到已有的人、登记为新人，或跳过。单张脸仍会按阈值自动判断是否同一人。也可以点「标为同一人」手动合并，或点某张缩略图拆出/删除。每个人可单独关掉「启用自动隐藏」：仍会识别，但不会藏窗口。勾选「加入黑名单」后，识别到此人会向已配置的消息渠道发送通知。
...
5. **消息渠道**：配置钉钉群机器人（关键词 / 加签 / IP 三种安全设置）、钉钉机器人应用、飞书，或自定义 HTTP 回调（可设请求头和参数）。黑名单人物出现时自动发送。
6. **模拟触发**：不靠人脸，直接走一遍隐藏/打开逻辑，方便调窗口策略。

识别设置里可以改匹配阈值、连续确认帧、冷却时间，以及是否「上传时自动把同一人归到一组」。也可勾选开机启动（登录后在托盘运行）。阈值越高越不容易误切或误归并。「监控时自动登记未知人脸」开启后，画面里出现没登记过的人脸会自动入库（命名为「未知人脸 N」），默认关闭自动隐藏、不加入黑名单、不会触发切窗口，也不会通知，之后可到人脸库改名或合并。侧栏底部可在中文 / English 之间切换，语言会记住。
